...
#define RtlRemoveTrailingPathSeparators(Flags, Path) \
    ...
    __declspec( selectany ) extern const UNICODE_STRING RtlNtPathSeparatorString = RTL_CONSTANT_STRING( ( PWSTR )L"\\" );
    __declspec( selectany ) extern const UNICODE_STRING RtlDosPathSeparatorsString = RTL_CONSTANT_STRING( ( PWSTR )L"\\/" );
    __declspec( selectany ) extern const UNICODE_STRING RtlAlternateDosPathSeparatorString = RTL_CONSTANT_STRING( ( PWSTR )L"/" );
#else
...
#define RTL_IS_DOS_PATH_SEPARATOR(ch_) ((ch_) == '\\' || ((ch_) == '/'))
    ...
    inline bool RtlIsDosPathSeparator( WCHAR ch )
    {
        return RTL_IS_DOS_PATH_SEPARATOR( ch );
    }
#endif
...
#if defined(__cplusplus) && defined(_MSC_VER) && _MSC_VER >= 1100 // VC5 for bool
    inline bool RtlIsNtPathSeparator( WCHAR ch )
    {
        return RTL_IS_NT_PATH_SEPARATOR( ch );
    }
#endif
